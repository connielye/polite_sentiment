...
class BiLSTM (nn.Module):
    def __init__(self, glove_embedding, vocab_size, embedding_dim, n_hidden, num_class):
        ...
        self.W = nn.Parameter(torch.randn([n_hidden * 2, num_class]).type(torch.FloatTensor))
        self.b = nn.Parameter(torch.randn([num_class]).type(torch.FloatTensor))
    ...
    def forward(self, X):
        ...
        output = output[-1]
        out = torch.mm(output, self.W)+self.b
        return out, final_hidden_state, final_cell_state
...
def train_model(model, learning_rate, epochs, training_data, dev_data, val_data, output_path, record_path):
    ...
    for epoch in range(epochs):
        ...
        for (training_batch, target_batch) in training_data:
            output,_,_ = model.forward(training_batch)
        ...
        if (epoch+1) % 2 == 0:
            ...
            for (dev_input_batch, dev_target_batch) in dev_data:
                predict, _, _ = model.forward(dev_input_batch)
    ...
    for (val_input_batch, val_target_batch) in val_data:
        predict, _, _ = model.forward(val_input_batch)
        predict = predict.data.max(1, keepdim=True)[1]
